...
function MovieCard(props) {
    ...
    return (
        <div className="each mb-10 m-2 shadow-xl border-gray-800 static cursor-pointer ">
            {
                props.movie.poster_path === null ? (
                    <img onClick={() => redirectMovieDetail(props.movie.id)} className="w-full h-5/6 object-contain rounded-t-2xl" src={imageNotAvailable} alt="imageNotFound" />      
                ) : (
                    <img onClick={() => redirectMovieDetail(props.movie.id)} className="w-full rounded-t-2xl" src={`https://image.tmdb.org/t/p/w300/${props.movie.poster_path}`} alt="imageNotFound" />       
                )
            }
            ...
            <span className="text-sm block py-2 font-bold border-gray-400 mb-2">{props.movie.title}</span>
            </div>
        </div>        
    )
}

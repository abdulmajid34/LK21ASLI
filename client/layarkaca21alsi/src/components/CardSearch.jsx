...
function CardSearch({searchData}) {
  ...
  return (
    <>
        <div className='w-80 mt-4 h-5/6 group transition duration-500 ease-in-out hover:scale-110 relative cursor-pointer'>
            <div className='my-7 px-4 w-full h-full'>
            {
                searchData.poster_path === null ? (
                    <img onClick={() => redirectMovieDetail(searchData.id)} className=' object-cover mb-48 border-4 text-black  rounded-2xl' src={imageNotAvailabel} alt="imageNotAvailabel" />
                ) : (
                    <img onClick={() => redirectMovieDetail(searchData.id)} className=' object-cover rounded-2xl' src={`https://image.tmdb.org/t/p/w500/${searchData.poster_path}`} alt="imageNotAvailable" />
                )
            }
            <div className=' top-72 absolute opacity-0 px-4 group-hover:opacity-100 w-full left-0 right-0 bottom-0 h-2/6 bg-white transition duration-500 bg-opacity-75 rounded-xl'>
                <div className="w-full h-full flex flex-col justify-start overflow-y-scroll items-start p-4">
                    <div className=' py-2'>
                        <span className=' font-bold'>{searchData.title}</span>
                    </div>
                    <div className=''>
